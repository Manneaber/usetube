...
export default async function getChannelVideos(id: string, published_after?: Date) {
    try {
        ...
        for(let i = 0; i < channel.length; i++) {
            const video: Video = await formatVideo(channel[i], false);
        ...
        while(token) {
            try {
                const data = await getData('https://www.youtube.com/youtubei/v1/browse?key='+apikey+'&token='+token);
                const newVideos: any = data.items;
                token = data.token;
                for(let i = 0; i < newVideos.length; i++) {
                    const video: Video = await formatVideo(newVideos[i], false);
                    if(video) {
                        if(published_after) {
