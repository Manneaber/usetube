...
export default async function getPlaylistVideos(id: string, speedDate?: boolean): Promise<Video[]> {
    try {
        ...
        while(token) {
            try {
                const nextData: any = await getData('https://www.youtube.com/youtubei/v1/browse?key='+apikey+'&token='+token);
                const nextVideos: any = nextData.items;
