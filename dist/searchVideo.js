...
async function searchVideo(terms, token, apikey) {
    try {
        ...
        if (!token) {
            let data = await getData_1.default('https://m.youtube.com/results?videoEmbeddable=true&search_query=' + encodeURI(terms));
            apikey = data.apikey;
            token = findVal_1.default(data, 'token');
            items = findVal_1.default(data, 'itemSectionRenderer').contents;
        }
        // more videos
            ...
            let data = await getData_1.default('https://www.youtube.com/youtubei/v1/search?key=' + apikey + '&token=' + token);
            items = findVal_1.default(data.items, 'contents');
            token = data.token;
        }
        for (let i = 0; i < items.length; i++) {
            let formated = await formatVideo_1.default(items[i], true);
            if (formated) {
                if (formated.id === 'didyoumean') {
                    didyoumean = formated.title;
                }
